"""Canonical PGP word list (Zimmermann, public domain).

Two parallel lists of 256 words each. Even-indexed bytes map to EVEN_WORDS,
odd-indexed bytes map to ODD_WORDS. By alternating the lists per byte,
adjacent words always come from different vocabularies, which makes
mis-hearing or transposing impossible to confuse with a valid sequence.
...
assert len(EVEN_WORDS) == 256, f"EVEN_WORDS has {len(EVEN_WORDS)} entries"
assert len(ODD_WORDS) == 256, f"ODD_WORDS has {len(ODD_WORDS)} entries"


def bytes_to_words(data: bytes) -> list[str]:
    """Translate bytes to alternating even/odd PGP words."""
    out: list[str] = []
    for i, b in enumerate(data):
        table = EVEN_WORDS if (i % 2 == 0) else ODD_WORDS
        out.append(table[b])
    return out
